...
class BeginProofWidget extends ProofWidget {
  ...
  eq(other: BeginProofWidget,): boolean {
    return this.profile.id === other.profile.id && this.display === other.display
      && this.linktext === other.linktext && this.sourcePath == other.sourcePath;
  }
...
export const createProofDecoration = (plugin: LatexReferencer,) =>
            ...
            enter(node,) {
              ...
              if (text.startsWith(settings.beginProof,)) {
                ...
                if (state.sliceDoc(node.to + 1, node.to + 2,) == '@') { // Check if "`\begin{proof}`@[[link]]" or "`\begin{proof}[display]`@[[link]]"
                  ...
                  const afterAfterNext = node.node.nextSibling?.nextSibling?.nextSibling
                    ?.nextSibling;
                  if (
                    next?.name === LINK_BEGIN && afterNext?.name === LINK
                    && afterAfterNext?.name === LINK_END
                  ) {
