...
export const createProofProcessor =
  (plugin: LatexReferencer,) => (element: HTMLElement, context: MarkdownPostProcessorContext,) => {
...
function parseAtSignLink(
  codeEl: HTMLElement,
): { atSign: ChildNode; links: HTMLElement[]; } | undefined {
  ...
  if (afterNext) {
    if (
      ...
      && afterAfterNext instanceof HTMLElement
      && afterAfterNext.matches('a.mathLink-internal-link',)
    ) {
  ...
  update(): void {
    ...
    if (this.atSignParseResult) {
      ...
      newEl.replaceChildren(
        profile.body.proof.linkedBeginPrefix,
        ...links,
        profile.body.proof.linkedBeginSuffix,
      );
