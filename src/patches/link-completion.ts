  ...
  plugin.register(around(prototype, {
    renderSuggestion(old,) {
      return function(item: any, el: HTMLElement,) {
        ...
        if (
          plugin.extraSettings.showTheoremTitleinBuiltin && item.type === 'block'
          && item.node.type === 'callout' && isTheoremCallout(plugin, item.node.callout.type,)
        ) {
          let title: string = item.node.children.find((child: any,) =>
            child.type === 'callout-title'
          ...
          const parsed = _readTheoremCalloutSettings({
            type: item.node.callout.type,
            metadata: item.node.callout.data,
          }, plugin.extraSettings.excludeExampleCallout,);
          if (parsed) {
            ...
            const formattedTitle = formatTitle(
              plugin,
              item.file as TFile,
              resolveSettings({ type, number, title, }, plugin, item.file as TFile,),
              true,
            );
            renderInSuggestionTitleEl(el, (suggestionTitleEl,) => {
              el.addClass('math-booster', 'suggestion-item-theorem-callout',);
...
function renderInSuggestionTitleEl(
  el: HTMLElement,
  cb: (suggestionTitleEl: HTMLElement,) => void,
) {
  // setTimeout(() => {
