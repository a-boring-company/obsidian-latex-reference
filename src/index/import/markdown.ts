...
export function markdownImport(
  ...
  for (let index = 0; index < metaheadings.length; index++) {
    ...
    const end = index == metaheadings.length - 1
      ? lines.length - 1
      : metaheadings[index + 1].position.start.line - 1;
  ...
  if (
    (!firstSection && !empty)
    || (firstSection && !emptylines(lines, 0, firstSection[1].$position.start,))
  ) {
