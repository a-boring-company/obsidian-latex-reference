...
export class MarkdownPage implements File, Linkbearing, Indexable {
  ...
  public getBlockByLineNumber(line: number,) {
    const section = this.$sections.find((section,) =>
      section.$position.start <= line && line <= section.$position.end
    );
    const block = section?.$blocks.find((block,) =>
      block.$position.start <= line && line <= block.$position.end
    );
    return block;
  }

  public getBlockByOffset(offset: number,): MarkdownBlock | undefined {
...
export class MarkdownSection implements Indexable, Linkable, Linkbearing {
  ...
  static readableId(file: string, title: string, ordinal: number,): string {
    const first8 = title.substring(0, Math.min(title.length, 8,),).replace(
      /[^A-Za-z0-9-_]+/gi,
      '-',
    );
...
export class TheoremCalloutBlock extends MathBlock implements Linkbearing {
  ...
  get $printName(): string {
    return this.$theoremSubtitle
      ? `${this.$theoremMainTitle} (${this.$theoremSubtitle})`
      : this.$theoremMainTitle;
  }
