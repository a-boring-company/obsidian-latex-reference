...
import { Indexable, Linkable, LINKBEARING_TYPE, } from './typings/indexable';
...
export class MathIndex {
  ...
  private _recursiveStore<T extends Indexable,>(
    ...
    substorer?.(
      object,
      (incoming, subindex,) => this._recursiveStore(incoming, revision, subindex, object,),
    );
  }
  ...
  public updateNames(file: TFile,) {
    ...
    while (block = this.load(`${file.path}/block${blockOrdinal++}`,)) {
      if (TheoremCalloutBlock.isTheoremCalloutBlock(block,)) {
        ...
        if (block.$manualTag) {
          ...
          !settings.numberOnlyReferencedEquations
          || block.$link && this.isLinked(block as Linkable,)
        ) {
          block.$index = equationCount;
          printName = '(' + eqPrefix
            + CONVERTER[settings.eqNumberStyle](equationNumberInit + equationCount,) + eqSuffix
            + ')';
          equationCount++;
        }
    ...
    if (MarkdownPage.isMarkdownPage(page,)) {
      if (mainTheorem) {
        ...
        if (
          !resolvedSettings.ignoreMainTheoremCalloutWithoutTitle || mainTheorem.$theoremSubtitle
        ) {
  ...
  formatMathLink(
    ...
    key: 'refFormat' | 'noteMathLinkFormat',
  ): string {
    ...
    if (refFormat == '[title] if title exists, [type] [number] otherwise') {
      return resolvedSettings.title
        ? resolvedSettings.title
        : formatTitleWithoutSubtitle(this.plugin, file, resolvedSettings,);
    }
    ...
    return resolvedSettings.title
      ? `${resolvedSettings.title} (${typePlusNumber})`
      : typePlusNumber;
  }
