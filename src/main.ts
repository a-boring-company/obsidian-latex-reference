...
import {
  DependencyNotificationModal,
  MigrationModal,
  PluginSplitNoticeModal,
  RenameNoticeModal,
} from 'notice';
...
import {
  ...
  ExtraSettings,
  MathContextSettings,
  UNION_TYPE_EXTRA_SETTING_KEYS,
  UNION_TYPE_MATH_CONTEXT_SETTING_KEYS,
} from 'settings/settings';
import { MathSettingTab, } from 'settings/tab';
import { createTheoremCalloutPostProcessor, } from 'theorem-callouts/renderer';
import { createTheoremCalloutsField, TheoremCalloutInfo, } from 'theorem-callouts/state-field';
import { createTheoremCalloutNumberingViewPlugin, } from 'theorem-callouts/view-plugin';
import {
  getMarkdownPreviewViewEl,
  getMarkdownSourceViewEl,
  isPluginOlderThan,
} from 'utils/obsidian';
import {
  getProfile,
...
export default class LatexReferencer extends Plugin {
  ...
  async onload() {
    ...
    this.app.workspace.onLayoutReady(async () => {
      const dependenciesOK = Object.keys(this.dependencies,).every((id,) =>
        this.checkDependency(id,)
      );
  ...
  async loadSettings() {
    this.settings = { [VAULT_ROOT]: structuredClone(DEFAULT_SETTINGS,), };
    this.extraSettings = structuredClone(DEFAULT_EXTRA_SETTINGS,);
    this.excludedFiles = [];
