  ...
  setScope() {
    ...
    this.scope.register([this.plugin.extraSettings.modifierToJump,], 'Enter', () => {
      ...
      openFileAndSelectPosition(
        ...
        file,
        item.$pos,
        ...LEAF_OPTION_TO_ARGS[this.plugin.extraSettings.suggestLeafOption],
      );
    ...
    if (this.plugin.extraSettings.showModifierInstruction) {
      this.parent.setInstructions([
        ...
        {
          command: `${
            getModifierNameInPlatform(this.plugin.extraSettings.modifierToNoteLink,)
          } + ↵`,
          purpose: 'to insert link to note',
        },
        {
          command: `${getModifierNameInPlatform(this.plugin.extraSettings.modifierToJump,)} + ↵`,
          purpose: 'to jump',
        },
      ],);
    }
        ...
        if (this.plugin.extraSettings.searchLabel) {
          ...
          if (file instanceof TFile) {
            const resolvedSettings = resolveSettings(
              (block as TheoremCalloutBlock).$settings,
              this.plugin,
              file,
            );
