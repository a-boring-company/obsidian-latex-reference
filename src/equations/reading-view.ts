...
export const createEquationNumberProcessor =
  (plugin: LatexReferencer,) => async (el: HTMLElement, ctx: MarkdownPostProcessorContext,) => {
 ...
 */
function preprocessForPdfExport(
  plugin: LatexReferencer,
  el: HTMLElement,
  ctx: MarkdownPostProcessorContext,
) {
  try {
    const topLevelMathDivs = el.querySelectorAll<HTMLElement>(
      ':scope > div.math.math-block > mjx-container.MathJax[display="true"]',
    );

    const page = plugin.indexManager.index.getMarkdownPage(ctx.sourcePath,);
    if (!page) {
      new Notice(
        `${plugin.manifest.name}: Failed to fetch the metadata for PDF export; equation numbers will not be displayed in the exported PDF.`,
      );
      return;
    }
    ...
    if (topLevelMathDivs.length != equationIndex) {
      new Notice(
        `${plugin.manifest.name}: Something unexpected occured while preprocessing for PDF export. Equation numbers might not be displayed properly in the exported PDF.`,
      );
    }
  } catch (err) {
    new Notice(
      `${plugin.manifest.name}: Something unexpected occured while preprocessing for PDF export. See the developer console for the details. Equation numbers might not be displayed properly in the exported PDF.`,
    );
    console.error(err,);
  }
}
...
export class EquationNumberRenderer extends MarkdownRenderChild {
  ...
  update() {
    ...
    const equation = id
      ? this.index.getEquationBlock(id,)
      : this.getEquationCacheCaringHoverAndEmbed();
    if (!equation) return;
