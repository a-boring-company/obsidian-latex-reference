...
import { editorInfoField, finishRenderMath, MarkdownView, TFile, } from 'obsidian';
  ...
  return ViewPlugin.fromClass(
    class implements PluginValue {
      ...
      update(update: ViewUpdate,) {
        ...
        if (
          update.transactions.some(tr => tr.effects.some(effect => effect.is(forceUpdateEffect,)))
        ) {
          // index updated
          this.settings = resolveSettings(undefined, plugin, this.file,);
          this.updatePage(this.file,).then((updatedPage,) =>
            this.updateEquationNumber(update.view, updatedPage,)
          );
        } else if (update.geometryChanged) {
          if (this.page) this.updateEquationNumber(update.view, this.page,);
          else {this.updatePage(this.file,).then((updatedPage,) =>
              this.updateEquationNumber(update.view, updatedPage,)
            );}
        }
      }
      ...
      async updateEquationNumber(view: EditorView, page: MarkdownPage,) {
        const mjxContainerElements = view.contentDOM.querySelectorAll<HTMLElement>(
        ...
        for (const mjxContainerEl of mjxContainerElements) {
          ...
          if (block.$printName !== null) {
            mjxContainerEl.setAttribute(
              'data-equation-print-name',
              block.$printName,
            );
