...
export class TheoremCalloutSettingsHelper {
  ...
  makeSettingPane() {
    ...
    new Setting(contentEl,)
      ...
      .addDropdown((dropdown,) => {
        for (const id of THEOREM_LIKE_ENV_IDs) {
          const envName = formatTheoremCalloutType(this.plugin, {
            type: id,
            profile: this.defaultSettings.profile,
          },);
        ...
        const labelPane = this.plugin.extraSettings.setLabelInModal
          ? new Setting(contentEl,).setName('Pandoc label',)
          : undefined;
        const labelPrefixEl = labelPane?.controlEl.createDiv({
          text: THEOREM_LIKE_ENVs[this.settings.type as TheoremLikeEnvID].prefix + ':'
            + (this.defaultSettings.labelPrefix ?? ''),
        },);
        ...
        dropdown.onChange((value,) => {
          ...
          if (labelPrefixEl) {
            labelPrefixEl.textContent =
              THEOREM_LIKE_ENVs[this.settings.type as TheoremLikeEnvID].prefix + ':';
    ...
    }, this.defaultSettings.fold ?? this.plugin.extraSettings.foldDefault,);
  }
}
  ...
  extends Component
{
  ...
  addSliderSetting(
    name: NumberKeys<SettingsType>,
    limits: { min: number; max: number; step: number | 'any'; },
    prettyName: string,
    description?: string,
  ): Setting {
    const setting = new Setting(this.contentEl,).setName(prettyName,);
...
export class MathContextSettingsHelper extends SettingsHelper<MathContextSettings> {
  ...
  onload() {
    ...
    const styleSetting = this.addDropdownSetting(
      'theoremCalloutStyle',
      THEOREM_CALLOUT_STYLES,
      'Style',
    );
    ...
    this.addToggleSetting(
      'theoremCalloutFontInherit',
      'Don\'t override the app\'s font setting when using preset styles',
      'You will need to reload the note to see the changes.',
    );
    this.addTextSetting(
      'titleSuffix',
      'Title suffix',
      'Ex) "" > Definition 2 (Group) / "." > Definition 2 (Group).',
    );
    this.addTextSetting(
      'labelPrefix',
      'Pandoc label prefix',
      'Useful for ensuring no label collision. Ex) When "Pandoc label prefix" = "foo:", A theorem with "Pandoc label" = "bar" is assigned "thm:foo:bar."',
    );
    ...
    this.addTextSetting(
      ...
      'Use property as source',
      'If set, use this property as the source of prefix inference. If not set, the note title will be used as the source.',
    );
    this.addTextSetting('inferNumberPrefixRegExp', 'Regular expression for parsing',);
    this.addTextSetting(
      'numberPrefix',
      'Manual prefix',
      'Even if "Infer prefix from note title or properties" is turned on, the inferred prefix will be overwritten by the value set here.',
    );
    ...
    this.addTextSetting(
      'numberDefault',
      'Default value for the "Number" field of "Insert theorem callout" modal',
    );
    ...
    this.addToggleSetting(
      'ignoreMainTheoremCalloutWithoutTitle',
      'Ignore a "main" theorem callout without its own title',
    );
    ...
    this.addTextSetting(
      'eqNumberPrefix',
      'Manual prefix',
      'Even if "Infer prefix from note title" is turned on, the inferred prefix will be overwritten by the value set here.',
    );
    ...
    this.addHeading('Search & link auto-completion - general',)
      .then(async (setting,) => {
        ...
        await MarkdownRenderer.render(
          this.plugin.app,
          '**NOTE:** If you have the [**Quick Preview**](https://github.com/RyotaUshio/obsidian-quick-preview) plugin installed, holding down `Alt`/`Option` _(by default)_ will trigger a quick preview of the selected suggestion with the context around it.',
          setting.descEl,
          '',
          this,
        );
  ...
  addProfileSetting(defaultValue?: string,): Setting {
    const profileSetting = this.addDropdownSetting(
      'profile',
      Object.keys(this.plugin.extraSettings.profiles,),
      'Profile',
      'A profile defines the displayed name of each environment.',
      defaultValue,
    );
...
export class ExtraSettingsHelper extends SettingsHelper<ExtraSettings> {
  onload(): void {
    ...
    this.addToggleSetting(
      'noteTitleInTheoremLink',
      'Show the note title at the head of a link to a theorem',
      'If turned on, a link to "Theorem 1" will look like "Note title > Theorem 1".',
    );
    this.addToggleSetting(
      'noteTitleInEquationLink',
      'Show the note title at the head of a link to an equation',
      'If turned on, a link to "Eq.(1)" will look like "Note title > Eq.(1)".',
    );
    this.addToggleSetting(
      'excludeExampleCallout',
      'Don\'t treat "> [!example]" as a theorem callout',
      'If turned on, a callout of the form "> [!example]" will be treated as Obsidian\'s built-in "Example" callout, and you will need to type "> [!exm]" instead to insert a theorem callout of "Example" type.',
    );
    this.addToggleSetting(
      'showTheoremCalloutEditButton',
      'Show an edit button on a theorem callout',
    );
    this.addToggleSetting(
      'setOnlyTheoremAsMain',
      'If a note has only one theorem callout, automatically set it as main',
      'Regardless of this setting, putting "%% main %%" or "%% main: true %%" in a theorem callout will set it as main one of the note, which means any link to that note will be displayed with the theorem\'s title. Enabling this option implicitly sets a theorem callout as main when it\'s the only one in the note.',
    );
    this.addToggleSetting(
      'setLabelInModal',
      'Show LaTeX/Pandoc label input form in theorem callout insert/edit modal',
    );
    this.addToggleSetting(
      'enableProof',
    ...
    this.addSliderSetting(
      ...
      'Specify how many items are suggested at one time. Set it to a smaller value if you have a performance issue when equation suggestions with math rendering on.',
    );
    this.addToggleSetting(
      'renderMathInSuggestion',
      'Render math in equation suggestions',
      'Turn this off if you have a performance issue and reducing the number of suggestions doesn\'t fix it.',
    );
    this.addDropdownSetting(
      'searchMethod',
      ['Fuzzy', 'Simple',],
      'Search method',
      'Fuzzy search is more flexible, but simple search is lighter-weight.',
    );
    this.addToggleSetting('searchLabel', 'Include theorem callout label for search target',);
    this.addSliderSetting(
      'upWeightRecent',
      { min: 0, max: 0.5, step: 0.01, },
      'Up-weight recently opened notes by',
      'It takes effect only if "Search only recently opened notes" is turned off.',
    );
    this.addDropdownSetting(
      'modifierToJump',
      ['Mod', 'Ctrl', 'Meta', 'Shift', 'Alt',],
      'Modifier key for jumping to suggestion',
      'Press Enter and this modifier key to jump to the currently selected suggestion. Changing this option requires to reloading '
        + this.plugin.manifest.name + ' to take effect.',
    );
    this.addDropdownSetting(
      'modifierToNoteLink',
      ['Mod', 'Ctrl', 'Meta', 'Shift', 'Alt',],
      'Modifier key for insert link to note',
      'Press Enter and this modifier key to insert a link to the note containing the currently selected item. Changing this option requires to reloading '
        + this.plugin.manifest.name + ' to take effect.',
    );
    this.addToggleSetting(
      'showModifierInstruction',
      'Show modifier key instruction',
      'Show the instruction for the modifier key at the bottom of suggestion box. '
        + `Changing this option requires to reloading ${this.plugin.manifest.name} to take effect.`,
    );
    ...
    this.addDropdownSetting(
      ...
      LEAF_OPTIONS,
      'Opening option',
      'Specify how to open the selected suggestion.',
    );

    this.addHeading('Enhance Obsidian\'s built-in link auto-completion (experimental)',)
      .setDesc(
        'Configure how this plugin modifies the appearance of Obsidian\'s built-in link auto-completion (the one that pops up when you type "[["). This feature dives deep into Obsidian\'s internals, so it might break when Obsidian is updated. If you encounter any issue, please report it on GitHub.',
      );
    this.addToggleSetting('showTheoremTitleinBuiltin', 'Show theorem title',);
    this.addToggleSetting(
      'showTheoremContentinBuiltin',
      'Show theorem content',
      'Only effective when "Show theorem title" is turned on.',
    );

    this.addHeading('Configure this plugin\'s custom editor link auto-completion',)
      .setDesc(
        `It is recommended to turn off unnecessary auto-completions to improve performance.`,
      );

    this.addTextSetting(
      'autocompleteDvQuery',
      'Dataview query for editor link auto-completion',
      'Only LIST queries are supported.',
    );
    ...
    this.addSliderSetting(
      ...
      { min: 1, max: 10, step: 1, },
      'Indexer threads',
      'The maximum number of thread used for indexing.',
    );
    this.addSliderSetting(
      'importerUtilization',
      { min: 0.1, max: 1.0, step: 0.01, },
      'Indexer CPU utilization',
      'The CPU utilization that indexer threads should use.',
    );
  }
}

function addFoldOptionSetting(
  el: HTMLElement,
  name: string,
  onChange: (fold: FoldOption,) => any,
  defaultValue?: FoldOption,
) {
