...
import { MathContextSettingsHelper, TheoremCalloutSettingsHelper, } from 'settings/helper';
import {
  DEFAULT_SETTINGS,
  MathContextSettings,
  MathSettings,
...
export class TheoremCalloutModal extends MathSettingModal<MathSettings> {
  ...
  resolveDefaultSettings(currentFile: TAbstractFile,) {
    ...
    if (this.currentCalloutSettings === undefined) {
      this.defaultSettings = resolveSettings(
        this.currentCalloutSettings,
        this.plugin,
        currentFile,
      );
    } else {
  ...
  onOpen(): void {
    ...
    const helper = new TheoremCalloutSettingsHelper(
      contentEl,
      this.settings,
...
export class ContextSettingModal extends MathSettingModal<MathContextSettings> {
  ...
  onOpen(): void {
    ...
    const defaultSettings = this.file.parent
      ? resolveSettings(undefined, this.plugin, this.file.parent,)
      : DEFAULT_SETTINGS;

    const contextSettingsHelper = new MathContextSettingsHelper(
      contentEl,
...
export class ExcludedFileManageModal extends Modal {
  ...
  async newDisplay() {
    ...
    new Setting(contentEl,)
      .setName(
        'The files/folders in this list and their descendants will be excluded from suggestion for local settings.',
      )
      .addButton((btn,) => {
        btn.setIcon('plus',)
          .onClick((event,) => {
