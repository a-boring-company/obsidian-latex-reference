...
export class ManageProfileModal extends Modal {
  constructor(
    ...
    public helper: MathContextSettingsHelper,
      ...
      new Setting(contentEl,)
        ...
        .addButton((editButton,) => {
          ...
          editButton.setIcon('copy',)
            .setTooltip('Copy',)
            .onClick(() => {
              const copied = structuredClone(this.plugin.extraSettings.profiles[id],) as Profile;
              if (copied) {
...
class EditProfileModal extends Modal {
  ...
  onOpen() {
    ...
    new Setting(contentEl,)
      .setName('Tags',)
      .setDesc(
        'Comma-separated list of tags. Only lower-case alphabets or hyphens are allowed. Each tag is converted into a CSS class ".theorem-callout-<tag>".',
      )
      .addText((text,) => {
        text.setValue(this.profile.meta.tags.join(', ',),)
          .onChange((value,) => {
            const tags = value.split(',',).map((item,) => item.trim());
            if (tags.every((tag,) => !tag || Boolean(tag.match(/^[a-z-]+$/,),))) {
    ...
    const linkedProofHeading = new Setting(contentEl,)
      ...
      .setDesc(
        `For example, you can render \`${DEFAULT_SETTINGS.beginProof}\`@[[link to Theorem 1]] as "${
          DEFAULT_PROFILES[DEFAULT_SETTINGS.profile].body.proof.linkedBeginPrefix
        }Theorem 1${DEFAULT_PROFILES[DEFAULT_SETTINGS.profile].body.proof.linkedBeginSuffix}".`,
      )
...
class UpdateProfileModal extends Modal {
  constructor(
    public parent: ConfirmProfileDeletionModal,
  ...
  onOpen() {
    ...
    contentEl.createDiv({
      text: `The following ${this.affected.length > 1 ? this.affected.length : ''} local setting${
        this.affected.length > 1 ? 's are' : ' is'
...
function makeIdOfCopy(oldID: string, profiles: Record<string, Profile>,) {
  ...
  const ids = Object.keys(profiles,);
  const numbers: number[] = ids.map((id,) =>
    id.slice(oldID.length + 1,).match(/\(([1-9][0-9]*)\)/,)?.[1] ?? '0'
