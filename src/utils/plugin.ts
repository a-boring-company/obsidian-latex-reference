...
export function generateTheoremCalloutFirstLine(config: TheoremCalloutSettings,): string {
  const metadata = config.number === 'auto'
    ? ''
    : config.number === ''
    ? '|*'
    : `|${config.number}`;
  let firstLine = `> [!${config.type}${metadata}]${config.fold ?? ''}${
...
export function isTheoremCallout(plugin: LatexReferencer, type: string,) {
  ...
  return (THEOREM_LIKE_ENV_IDs as unknown as string[]).includes(type,)
    || (THEOREM_LIKE_ENV_PREFIXES as unknown as string[]).includes(type,) || type === 'math';
}
