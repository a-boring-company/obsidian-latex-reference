import { ChangeSet, EditorState, RangeSet, RangeValue, SelectionRange, } from '@codemirror/state';
import { SyntaxNodeRef, } from '@lezer/common';
import { editorLivePreviewField, EditorPosition, Loc, MarkdownView, } from 'obsidian';
...
export function nodeTextQuoteSymbolTrimmed(
  node: SyntaxNodeRef,
  state: EditorState,
  quoteLevel: number,
): string | undefined {
...
export function printChangeSet(changes: ChangeSet,) {
  changes.iterChanges(
    (fromA: number, toA: number, fromB: number, toB: number, inserted: any,) => {
      console.log(
        `${fromA}-${toA}: "${inserted.toString()}" inserted (${fromB}-${toB} in new state)`,
      );
    },
  );
}

export function rangeSetSome<T extends RangeValue,>(
  set: RangeSet<T>,
  predicate: (value: T, index: number, set: RangeSet<T>,) => unknown,
) {
