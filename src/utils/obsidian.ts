 ...
 */
export function iterDescendantFiles(
  file: TAbstractFile,
  callback: (descendantFile: TFile,) => any,
) {
...
export function getSectionCacheFromPos(
...
export function getSectionCacheFromMouseEvent(
  event: MouseEvent,
  type: string,
  view: EditorView,
  cache: CachedMetadata,
...
export function resolveLinktext(
  app: App,
  linktext: string,
  sourcePath: string,
): { file: TFile; subpathResult: HeadingSubpathResult | BlockSubpathResult | null; } | null {
...
export function getMarkdownPreviewViewEl(view: MarkdownView,) {
  return Array.from(view.previewMode.containerEl.children,).find((child,) =>
    child.matches('.markdown-preview-view',)
  );
}
...
export function isPdfExport(el: HTMLElement,): boolean {
  ...
  return (el.parentElement?.classList.contains('print',) ?? false)
    && el.matches('.markdown-preview-view.markdown-rendered',);
}
