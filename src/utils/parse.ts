...
export const THEOREM_CALLOUT_PATTERN = new RegExp(
  `> *\\[\\! *(?<type>${THEOREM_LIKE_ENV_IDs.join('|',)}|${
    THEOREM_LIKE_ENV_PREFIXES.join('|',)
  }|math) *(\\|(?<number>.*?))?\\](?<fold>[+-])?(?<title> .*)?`,
  'i',
);
 ...
 * @returns
 */
export function readTheoremCalloutSettings(
  line: string,
  excludeExample: boolean = false,
): MinimalTheoremCalloutSettings & { legacy: boolean; } | undefined {
  const rawSettings = line.match(THEOREM_CALLOUT_PATTERN,)?.groups as {
    type: string;
    number?: string;
    title?: string;
    fold?: string;
  ...
  if (type === 'math' && rawSettings.number) {
    // legacy format
    const settings = JSON.parse(rawSettings.number,) as MinimalTheoremCalloutSettings & {
      legacy: boolean;
    };
...
export function _readTheoremCalloutSettings(
  callout: { type: string; metadata: string; },
  excludeExample: boolean = false,
): { type: string; number: string; legacy: boolean; } | undefined {
  let type = callout.type;

  if (callout.type === 'math' && callout.metadata) {
    // legacy format
    const settings = JSON.parse(callout.metadata,) as MinimalTheoremCalloutSettings & {
      legacy: boolean;
    };
    settings.legacy = true;
    return settings;
...
export function parseLatexComment(line: string,): { nonComment: string; comment: string; } {
  ...
  if (match?.index !== undefined) {
    return {
      nonComment: line.substring(0, match.index,),
      comment: line.substring(match.index + 1,),
    };
  }
