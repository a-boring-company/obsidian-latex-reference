...
import {
  THEOREM_LIKE_ENV_ID_PREFIX_MAP,
...
class TheoremCalloutRenderer extends MarkdownRenderChild {
  ...
  constructor(
    ...
    this.addChild(
      this.observer = new MutationObservingChild(this.containerEl, (mutations,) => {
  ...
  isLivePreview(): boolean {
    return this.containerEl.closest('[src]',) === null
      && this.containerEl.closest('.markdown-source-view.is-live-preview',) !== null;
  }
  ...
  correctEmbedOrHoverPagePreview(
    block: (TheoremCalloutInfo & { blockId?: string; }) | null,
    info: TheoremCalloutInfo,
  ) {
    ...
    if (linktext) {
      ...
      const { file, subpathResult: result, } =
        resolveLinktext(this.app, linktext, this.context.sourcePath,) ?? {};
  ...
  findTheoremCalloutBlock(lineOffset: number = 0,): TheoremCalloutBlock | null {
    ...
    const block = page.getBlockByLineNumber(info.lineStart + lineOffset,)
      ?? page.getBlockByLineNumber(info.lineEnd + lineOffset,);
  ...
  getTheoremCalloutInfoFromEl(): TheoremCalloutInfo | null {
    ...
    let theoremSubtitleEl = this.containerEl.querySelector<HTMLElement>(
    ...
    if (theoremSubtitleEl === null) {
      ...
      if (titleInnerEl?.childNodes.length) {
        if (
          titleInnerEl.textContent !== capitalize(settings.type,)
          && titleInnerEl.textContent
            !== capitalize(THEOREM_LIKE_ENV_ID_PREFIX_MAP[settings.type as TheoremLikeEnvID],)
        ) {
          theoremSubtitleEl = createSpan({ cls: 'theorem-callout-subtitle', },);
  ...
  renderTitle(info: TheoremCalloutInfo, existingMainTitleEl?: HTMLElement | null,) {
    ...
    if (!titleInner) {
      throw Error(
        `${this.plugin.manifest.name}: Failed to find the title element of a theorem callout.`,
      );
    }
  ...
  addCssClasses(info: TheoremCalloutInfo,) {
    ...
    this.containerEl.toggleClass(
      `theorem-callout-${resolvedSettings.theoremCalloutStyle.toLowerCase()}`,
      resolvedSettings.theoremCalloutStyle != 'Custom',
    );
    this.containerEl.toggleClass(
      'theorem-callout-font-family-inherit',
      resolvedSettings.theoremCalloutStyle != 'Custom'
        && resolvedSettings.theoremCalloutFontInherit,
    );
  }
