...
export const createTheoremCalloutsField = (plugin: LatexReferencer,) =>
  StateField.define<RangeSet<TheoremCalloutInfo>>({
    ...
    update(value: RangeSet<TheoremCalloutInfo>, tr: Transaction,) {
      ...
      const updatedRanges = getTheoremCalloutInfos(
        plugin,
        tr.state,
        tr.newDoc,
        minChangedPosition,
...
function getTheoremCalloutInfos(
  plugin: LatexReferencer,
  state: EditorState,
  doc: Text,
  from: number,
  init: number,
): Range<TheoremCalloutInfo>[] {
  ...
  tree.iterate({
    ...
    enter(node,) {
      ...
      const settings = readTheoremCalloutSettings(
        text,
